...
{
    (USERIAL_DATABITS_8 | USERIAL_PARITY_NONE | USERIAL_STOPBITS_2),
    USERIAL_BAUD_115200
};
...
{
    char commchip[PROPERTY_VALUE_MAX];

    ALOGI("init");

    property_get("persist.commchip_vendor", commchip, "");
    if (!strcmp(commchip, "0x02df")) {
